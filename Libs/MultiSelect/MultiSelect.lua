...
do
	...
	local function GetTextList(self) -- return a table of all label texts
		local textList = {}
		for _, item in pairs(self.labels) do
			table.insert(textList, item.text:GetText())
		end
		return textList
	end
	...
	local function GetItemList(self) -- return a table of all label objects
		return self.labels
	end
	...
	local function Sort(self)
		table.sort(self.labels, function(l1, l2) return l1.text:GetText() < l2.text:GetText() end)
		self:RedrawFrame()
	end
	...
	local function Constructor() -- widget constructor
		...
		local scrollframe = CreateFrame("ScrollFrame", format("%s@%s@%s", widgetType, "ScrollFrame", tostring(self)),
